...
export function PageLens({ volumeId, page, pageData, initialQuestion='', onClose, onPageApplied }: { volumeId: string; page: number; pageData: MangaPage; initialQuestion?:string; onClose: () => void; onPageApplied: () => void }) {
  ...
  return (
    <aside className="tool-panel page-lens">
      ...
      <section className="lens-task lens-task--history"><div className="lens-task__title"><History size={19}/><div><span>RECOVER</span><h3>Past AI queries</h3></div></div><p>Reopen page analyses and focused explanations previously saved for this page.</p><div className="ai-history"><div className="section-rule"><span>{history.length} SAVED</span></div>{!history.length&&<p>No AI questions saved for this page yet.</p>}{history.map((item)=><article key={item.id}><div className="ai-history__head"><button onClick={()=>setOpenHistory(openHistory===item.id?null:item.id)}><span>{item.kind==='page'?'PAGE':'SELECTION'} · {new Date(item.created_at).toLocaleString()}</span><strong>{item.question}</strong></button><button title="Delete saved query" onClick={()=>removeHistory(item)}><Trash2 size={14}/></button></div>{openHistory===item.id&&<div className="ai-history__result"><small>{item.provider} · {item.model}{item.focus&&` · focus: ${item.focus}`}</small><p>{item.answer}</p>{item.kind==='selection'&&item.details?.breakdown?.map((part,index)=><dl key={index}><dt>{part.part}</dt><dd>{part.role}</dd></dl>)}{item.kind==='selection'&&item.details?.uncertainty&&<aside><strong>Uncertainty</strong>{item.details.uncertainty}</aside>}{item.kind==='page'&&item.details?.notes?.map((note,index)=><section key={index}><span>{note.type} · {Math.round(note.confidence*100)}%</span><strong>{note.title}</strong><p>{note.explanation}</p><blockquote>{note.evidence}</blockquote></section>)}</div>}</article>)}</div></section>
    </aside>
  )
}
