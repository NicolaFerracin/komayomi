...
import { api } from '../api'
import type { AiHistoryItem, ExplanationBody, LensAnalysis, LlmStatus, MangaPage, PageVisionProposal } from '../types'
...
export function PageLens({ volumeId, page, pageData, initialQuestion='', onClose, onPageApplied }: { volumeId: string; page: number; pageData: MangaPage; initialQuestion?:string; onClose: () => void; onPageApplied: () => void }) {
  ...
  async function followUp(item:AiHistoryItem){const request=(followUps[item.id]||'').trim();if(!request||!item.focus||!item.details?.sentence)return;setFollowing(item.id);setMessage('');try{await api.explainGrammar(item.details.sentence,item.focus,provider,request,{volume_id:volumeId,page_index:page,block_index:item.details.block_index??-1},item.id);setFollowUps((current)=>({...current,[item.id]:''}));setHistory(await api.aiHistory(volumeId,page));setOpenHistory(item.id)}catch(error){setMessage(error instanceof Error?error.message:'Could not continue this conversation')}finally{setFollowing(null)}}
  return (
    <aside className="tool-panel page-lens">
      ...
      <section className="lens-task lens-task--history"><div className="lens-task__title"><History size={19}/><div><span>RECOVER</span><h3>Past AI conversations</h3></div></div><p>Reopen and continue focused explanations, page analyses, and Meaning Checks saved for this page.</p><div className="ai-history"><div className="section-rule"><span>{history.length} SAVED</span></div>{!history.length&&<p>No AI conversations saved for this page yet.</p>}{history.map((item)=><article key={item.id}><div className="ai-history__head"><button onClick={()=>setOpenHistory(openHistory===item.id?null:item.id)}><span>{item.kind==='page'?'PAGE':item.kind==='comprehension'?'MEANING CHECK':'SELECTION'} · {new Date(item.created_at).toLocaleString()}</span><strong>{item.question}</strong></button><button title="Delete saved query" onClick={()=>removeHistory(item)}><Trash2 size={14}/></button></div>{openHistory===item.id&&item.kind==='comprehension'&&item.details?.evaluations&&<MeaningResults summary={item.details.summary||item.answer} evaluations={item.details.evaluations} answers={Object.fromEntries((item.details.answers||[]).map((answer)=>[answer.block_index,answer.interpretation]))}/>} {openHistory===item.id&&item.kind==='selection'&&<div className="ai-conversation">{(item.details?.turns||[{id:item.id,question:item.question,answer:{interpretation:item.answer,breakdown:item.details?.breakdown||[],uncertainty:item.details?.uncertainty||''},provider:item.provider,model:item.model,created_at:item.created_at}]).map((turn)=><div className="ai-conversation__turn" key={turn.id}><blockquote><span>YOU</span>{turn.question}</blockquote><Explanation answer={turn.answer} meta={`${turn.provider} · ${turn.model}`}/></div>)}{item.details?.sentence&&<><label><span>CONTINUE THIS CONVERSATION</span><textarea value={followUps[item.id]||''} onChange={(event)=>setFollowUps((current)=>({...current,[item.id]:event.target.value}))} placeholder="Ask a follow-up using everything discussed above…"/></label><button className="secondary-button" disabled={!provider||following===item.id||!(followUps[item.id]||'').trim()} onClick={()=>followUp(item)}>{following===item.id?<LoaderCircle className="spin" size={15}/>:<MessageCircle size={15}/>} Send follow-up</button></>}</div>}{openHistory===item.id&&item.kind==='page'&&<div className="ai-history__result"><small>{item.provider} · {item.model}</small><p>{item.answer}</p>{item.details?.notes?.map((note,index)=><section key={index}><span>{note.type} · {Math.round(note.confidence*100)}%</span><strong>{note.title}</strong><p>{note.explanation}</p><blockquote>{note.evidence}</blockquote></section>)}</div>}</article>)}</div></section>
    </aside>
  )
}

function Explanation({answer,meta}:{answer:ExplanationBody;meta:string}){return <div className="ai-history__result"><small>{meta}</small><p>{answer.interpretation}</p>{answer.breakdown.map((part,index)=><dl key={index}><dt>{part.part}</dt><dd>{part.role}</dd></dl>)}{answer.uncertainty&&<aside><strong>Uncertainty</strong>{answer.uncertainty}</aside>}</div>}
