...
export function PageLens({ volumeId, page, pageData, initialQuestion='', onClose, onPageApplied }: { volumeId: string; page: number; pageData: MangaPage; initialQuestion?:string; onClose: () => void; onPageApplied: () => void }) {
  ...
  return (
    <aside className="tool-panel page-lens">
      ...
      <section className="ai-history"><div className="section-rule"><span>PAST QUERIES / {history.length}</span></div>{!history.length&&<p>No AI questions saved for this page yet.</p>}{history.map((item)=><article key={item.id}><button onClick={()=>setOpenHistory(openHistory===item.id?null:item.id)}><span>{item.kind==='page'?'PAGE':'SELECTION'} · {new Date(item.created_at).toLocaleString()}</span><strong>{item.question}</strong></button>{openHistory===item.id&&<div><p>{item.answer}</p><small>{item.provider} · {item.model}{item.focus&&` · focus: ${item.focus}`}</small></div>}</article>)}</section>
