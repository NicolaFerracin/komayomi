...
export function PageNavigator({ pages, current, bookmarks, onChoose, onClose }: { pages: MangaPage[]; current: number; bookmarks: Set<number>; onChoose: (page: number) => void; onClose: () => void }) {
  ...
  const matches = useMemo(() => {
    ...
    const candidates = pages.map((page,index)=>({page,index})).filter(({index})=>!bookmarkedOnly||bookmarks.has(index))
  ...
  }, [pages, query, bookmarkedOnly, bookmarks])
  return <aside className="tool-panel page-navigator">
    ...
    <button className={bookmarkedOnly?'bookmark-filter active':'bookmark-filter'} onClick={()=>setBookmarkedOnly(!bookmarkedOnly)}><Bookmark size={14} fill={bookmarkedOnly?'currentColor':'none'}/> Bookmarks only <span>{bookmarks.size}</span></button>
