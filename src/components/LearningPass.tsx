...
export function LearningPass({volumeId,pageIndex,onClose,onLessonsChanged}:{volumeId:string;pageIndex:number;onClose:()=>void;onLessonsChanged:()=>void}){
  ...
  return <aside className="tool-panel learning-pass"><header><div><span className="eyebrow">KEEP ONLY WHAT TRAVELS</span><h2>Learning Pass</h2></div><button className="icon-button" aria-label="Close panel" onClick={onClose}><X size={19}/></button></header>
