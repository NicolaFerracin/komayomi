...
export function WorkflowGuide({onClose}:{onClose:()=>void}){
  return <aside className="tool-panel workflow-guide"><header><div><span className="eyebrow">THE PREFERRED METHOD</span><h2>Reading workflow</h2></div><button className="icon-button" onClick={onClose}><X size={19}/></button></header><div className="workflow-manifesto"><Route size={25}/><div><strong>The manga is the activity.</strong><p>Help should resolve friction now and make later pages easier—not turn reading into an endless review system.</p></div></div><div className="workflow-loop"><span>ATTEMPT</span><i>→</i><span>SELECTIVE HELP</span><i>→</i><span>CHECK</span><i>→</i><span>KEEP LITTLE</span><i>→</i><span>RECALL</span></div><div className="workflow-steps">{steps.map(({number,title,body,icon:Icon})=><article key={number}><b>{number}</b><Icon size={18}/><div><h3>{title}</h3><p>{body}</p></div></article>)}</div><section className="workflow-rules"><h3>Guardrails against burnout</h3><ul><li>No daily quota, streak, due date, or overdue pile.</li><li>Learn words in context—not isolated kanji.</li><li>Translation resolves a page; Learning Pass improves the next one.</li><li>Progress means needing less assistance, not completing more cards.</li><li>Skip anything obscure, obvious, or uninteresting.</li></ul></section><div className="workflow-reminder"><strong>Default page outcome</strong><span>Understand the scene, keep zero or one reusable idea, and continue reading.</span></div></aside>
}
