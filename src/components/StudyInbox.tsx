...
export function StudyInbox({ onClose, onOpenSource }: { onClose: () => void; onOpenSource:(volumeId:string,pageIndex:number)=>void }) {
  ...
  const [kind,setKind]=useState<'all'|SavedItem['kind']>('all')
  ...
  const visible=useMemo(()=>{const needle=query.trim().toLocaleLowerCase();return items.filter((item)=>(kind==='all'||item.kind===kind)&&(!needle||[item.text,item.reading,item.meaning,item.context,item.notes].some((value)=>value?.toLocaleLowerCase().includes(needle))))},[items,query,kind])
  ...
  return <div className="modal-backdrop" onMouseDown={onClose}><section className="study-inbox" role="dialog" aria-modal="true" aria-labelledby="study-inbox-title" onMouseDown={(event) => event.stopPropagation()}>
    <header><div><span className="eyebrow">LIGHTWEIGHT CAPTURE</span><h2 id="study-inbox-title">Study inbox</h2><p>Keep interesting words without creating daily review debt.</p></div><button className="icon-button" aria-label="Close panel" onClick={onClose}><X size={19}/></button></header>
    ...
    {!!items.length&&<div className="inbox-kinds">{(['all','vocabulary','sentence','grammar'] as const).map((value)=><button className={kind===value?'active':''} key={value} onClick={()=>setKind(value)}>{value}</button>)}</div>}
