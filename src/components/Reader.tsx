...
import { api } from '../api'
import type { Lesson, ReaderData, RubySpan, TextBlock } from '../types'
...
export function Reader({ data: initialData, onExit }: { data: ReaderData; onExit: () => void }) {
  ...
  const [recall,setRecall]=useState<{lesson:Lesson;selection:{text:string;ruby:RubySpan[];context:string;block:number}}|null>(null)
  ...
  function openLookup() {
    ...
    if(!closeEditor())return
    const selection={text:selectionAction.text,ruby:selectionAction.ruby,context:selectionAction.context,block:selectionAction.block}
    const learned=lessonMatches.find((lesson)=>(lesson.block_indices||[]).includes(selection.block)&&selection.text.includes(lesson.form))
    if(learned){setRecall({lesson:learned,selection});setSelectionAction(null);setLens(false);setNavigator(false);setSettings(false);return}
    openVocabulary(selection)
  }

  function openVocabulary(selection:{text:string;ruby:RubySpan[];context:string;block:number}){
    setLookup(selection);recentLookups.add({text:selection.text,context:selection.context,page:pageIndex,createdAt:new Date().toISOString()})
    api.assistance(data.volume_id,pageIndex,'lookup',selection.block).catch(()=>undefined)
