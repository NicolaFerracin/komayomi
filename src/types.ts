...
export type ExplanationBody={interpretation:string;breakdown:Array<{part:string;role:string}>;uncertainty:string}
export type GrammarExplanation = { id: string; thread_id:string;parent_id:string|null;sentence: string; focus: string; question: string | null; created_at: string; explanation: ExplanationBody; provider: string; model: string }
export type MeaningEvaluation={block_index:number;meaning_score:number;literal_score:number;verdict:string;literal_translation:string;natural_translation:string;contextual_meaning:string;correct:string[];missing:string[];added:string[];incorrect:string[];coverage?:Array<{japanese:string;meaning:string;status:'captured'|'partial'|'missing'|'incorrect';answer_evidence:string}>}
...
export type AiHistoryItem = {id:string;kind:'selection'|'page'|'comprehension';question:string;focus:string|null;answer:string;provider:string;model:string;created_at:string;details?:{sentence?:string;block_index?:number;interpretation?:string;breakdown?:Array<{part:string;role:string}>;uncertainty?:string;turns?:Array<{id:string;question:string;answer:ExplanationBody;provider:string;model:string;created_at:string}>;summary?:string;notes?:Array<{type:string;title:string;explanation:string;evidence:string;confidence:number}>;evaluations?:MeaningEvaluation[];answers?:Array<{block_index:number;japanese:string;interpretation:string}>}}
