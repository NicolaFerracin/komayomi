...
Judge meaning and communicative function, not word-for-word English equivalence. The reader may answer naturally or use an equivalent interjection from another language. For discourse markers and interjections, credit a different surface form when it plausibly performs the same function in the scene (for example acknowledgment, resolve, greeting, exertion, surprise, or attention-getting). Words such as okay, good, right, alright, hey, yo, or an equivalent non-English interjection are not automatically wrong merely because a dictionary gloss differs. When artwork was not supplied and a short interjection has several plausible functions, prefer partial credit for a contextually plausible interpretation rather than asserting one function with false certainty.

Distinguish omissions from contradictions. If an answer captures the topic and time frame but misses volition, direction, tense, or another important grammatical contribution, credit the supported units and mark only the absent contribution missing or partial. Never turn one omitted nuance into zero credit for an otherwise substantially aligned clause.
